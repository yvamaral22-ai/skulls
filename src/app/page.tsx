...
export default function DashboardPage() {
  ...
  // Fallback se não for Admin ou Staff (ex: Client ou erro de identificação)
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-[10px] uppercase font-bold tracking-[0.3em] flex items-center gap-2">
            ...
            {role === 'ADMIN' ? 'Controle Geral da Barbearia' : 'Painel do Profissional'}
          </p>
        </div>
        <Button className="h-14 w-full sm:w-auto font-black bg-primary text-black shadow-xl shadow-primary/20 hover:scale-[1.02] transition-transform" asChild>
          <Link href="/agenda">
      ...
      <div className="grid gap-4 md:gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
        <Card className="border-none bg-card shadow-2xl border-l-4 border-l-primary group hover:bg-primary/5 transition-colors">
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <CardTitle className="text-[10px] font-bold uppercase tracking-widest opacity-50">Clientes Hoje</CardTitle>
        ...
        <Card className="border-none bg-card shadow-2xl border-l-4 border-l-primary group hover:bg-primary/5 transition-colors">
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <CardTitle className="text-[10px] font-bold uppercase tracking-widest opacity-50">Cortes Realizados</CardTitle>
          ...
          <CardContent>
            ...
            <p className="text-[9px] text-muted-foreground uppercase mt-1">Total de serviços pagos</p>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <Card className="border border-border bg-card shadow-2xl overflow-hidden rounded-2xl">
          <CardHeader className="bg-primary/5 border-b border-border/50 px-6 py-6">
            <CardTitle className="text-xl font-headline text-primary uppercase">Meus Próximos Atendimentos</CardTitle>
          </CardHeader>
            ...
            <div className="divide-y divide-border/50 max-h-[450px] overflow-y-auto scrollbar-thin">
              {todayAppointments.length > 0 ? (
                ...
                <div className="text-center py-20 text-muted-foreground flex flex-col items-center gap-4">
                  ...
                  <p className="italic text-sm uppercase tracking-widest opacity-50">Nenhum agendamento para hoje.</p>
                </div>
              )}
            </div>
          </CardContent>
        ...
        <Card className="border border-border bg-card shadow-2xl rounded-2xl overflow-hidden">
          <CardHeader className="bg-primary/5 border-b border-border/50 px-6 py-6">
            <CardTitle className="text-xl font-headline text-primary uppercase">Atalhos do Sistema</CardTitle>
          </CardHeader>
            ...
            <Button variant="outline" className="h-20 justify-start gap-6 border-2 border-border hover:border-primary hover:bg-primary/5 transition-all text-left group rounded-2xl" asChild>
              <Link href="/agenda">
                ...
                <div className="min-w-0">
                  <p className="font-headline text-lg uppercase tracking-tight">Ver Minha Agenda</p>
                  <p className="text-[10px] uppercase font-bold opacity-40 tracking-widest">Confira seus horários</p>
                </div>
              </Link>
            </Button>
            {role === 'ADMIN' && (
                ...
                <Button variant="outline" className="h-20 justify-start gap-6 border-2 border-border hover:border-primary hover:bg-primary/5 transition-all text-left group rounded-2xl" asChild>
                  <Link href="/reports">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-[10px] uppercase font-bold opacity-40 tracking-widest">Ganhos e faturamento bruto</p>
                    </div>
                  </Link>
                </Button>
                <Button variant="outline" className="h-20 justify-start gap-6 border-2 border-border hover:border-primary hover:bg-primary/5 transition-all text-left group rounded-2xl" asChild>
                  <Link href="/staff">
                    ...
                    <div className="min-w-0">
                      <p className="font-headline text-lg uppercase tracking-tight">Gestão da Equipe</p>
                      <p className="text-[10px] uppercase font-bold opacity-40 tracking-widest">Controle de profissionais</p>
                    </div>
                  </Link>
                </Button>
              </>
